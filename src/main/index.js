...
async function saveMarkdownFile(filePath, content, defaultFolderPath = null) {
  ...
  const rootPath =
    defaultFolderPath && isPathInside(defaultFolderPath, normalized)
      ? path.resolve(defaultFolderPath)
      : null
...
async function saveMarkdownFileAs(content, defaultFolderPath = null) {
  const defaultPath = defaultFolderPath
    ? path.join(path.resolve(defaultFolderPath), 'Untitled.md')
    : 'Untitled.md'
...
async function exportPng(html, filePath) {
  ...
  try {
    ...
    const height = Math.min(Math.max(dimensions.height, 600), 12000)
...
function normalizeRemoteImagePath(imageSavePath = 'images', webdav = {}) {
  ...
  remotePath = tryDecodeURIComponent(remotePath)
    .replace(/[?#].*$/g, '')
    .replace(/\\/g, '/')
    .replace(/^[A-Za-z]:\/?/g, '')
...
function isUrlInsideBase(url, baseUrl) {
  ...
  try {
    ...
    return (
      ...
      (!basePath || parsedUrl.pathname === basePath || parsedUrl.pathname.startsWith(`${basePath}/`))
    )
  } catch {
    return false
  }
...
async function resolveRemoteImageAssetUrl(markdownBasePath, source, settings) {
  ...
  if (/^https?:\/\//i.test(source)) {
    ...
    if (!response.ok) throw new Error(`WebDAV 图片读取失败：HTTP ${response.status}`)
  ...
  const baseDir = isMarkdownFile(markdownRemotePath)
    ? path.posix.dirname(markdownRemotePath)
    : markdownRemotePath
  const remotePath = source.startsWith('/')
    ? `/${trimSlashes(source)}`
    : path.posix.resolve(baseDir, source)
...
async function resolveImageAssetUrl(markdownBasePath, source, syncSettings = null) {
  ...
  if (/^file:/i.test(source)) {
    ...
    const baseDir = isMarkdownFile(resolvedBasePath)
      ? path.dirname(resolvedBasePath)
      : resolvedBasePath
...
function createMarkdownImagePath(imageFilePath, baseDir, preferredRelativePath, imageName) {
  if (preferredRelativePath && !path.isAbsolute(preferredRelativePath)) {
...
async function chooseClipboardImagePath(markdownFilePath, defaultFolderPath, imageSavePath = 'images') {
  ...
  if (path.isAbsolute(preferredPath)) {
    ...
    return {
      filePath,
      markdownPath: createMarkdownImagePath(filePath, baseDir, preferredPath, imageName)
    }
  }
...
async function listWebDavDirectory(settings, remotePath) {
  ...
  return parseWebDavResponses(xml)
    .map((item) => ({
      ...item,
...
async function renameRemoteFolderItem({ settings, folderPath, itemPath, newName }) {
  ...
  return {
    ...
    folder: await refreshRemoteMarkdownFolder(settings, folderPath),
    file: isMarkdownFile(targetPath)
      ? await readRemoteMarkdownFile(settings, createWebDavVirtualPath(targetPath))
      : null
  }
}
...
ipcMain.handle('document:open-remote', (_event, { filePath, settings }) =>
  readRemoteMarkdownFile(settings, filePath)
)
...
ipcMain.handle('document:refresh-remote-folder', (_event, { settings, folderPath }) =>
  refreshRemoteMarkdownFolder(settings, folderPath)
)
...
ipcMain.handle('document:create-remote-in-folder', (_event, options) =>
  createRemoteMarkdownFileInFolder(options)
)
ipcMain.handle('document:create-remote-folder-in-folder', (_event, options) =>
  createRemoteFolderInFolder(options)
)
ipcMain.handle('document:rename-remote-folder-item', (_event, options) => renameRemoteFolderItem(options))
ipcMain.handle('document:duplicate-remote-folder-file', (_event, options) =>
  duplicateRemoteMarkdownFile(options)
)
ipcMain.handle('document:delete-remote-folder-item', (_event, options) => deleteRemoteFolderItem(options))
ipcMain.handle('document:show-remote-folder-item', (_event, { settings, itemPath }) =>
  showRemoteFolderItem(settings, itemPath)
)
ipcMain.handle('document:create', (_event, folderPath) => createMarkdownFile(folderPath))
ipcMain.handle('document:save', (_event, { filePath, content, defaultFolderPath }) =>
  saveMarkdownFile(filePath, content, defaultFolderPath)
)
ipcMain.handle('document:save-as', (_event, { content, defaultFolderPath }) =>
  saveMarkdownFileAs(content, defaultFolderPath)
)
ipcMain.handle('document:save-remote', (_event, { settings, filePath, content }) =>
  saveRemoteMarkdownFile(settings, filePath, content)
)
...
ipcMain.handle('document:asset-url', (_event, { markdownFilePath, source, syncSettings }) =>
  resolveImageAssetUrl(markdownFilePath, source, syncSettings)
)
